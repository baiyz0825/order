...
**首次部署或全新安装时，系统会自动引导您完成初始化设置。**

### 初始化向导

访问管理后台时，如果系统检测到未初始化，会自动跳转到初始化向导页面：
...
1. **创建管理员账户** - 设置管理员的邮箱、姓名和密码
2. **配置店铺信息** - 填写店铺名称、营业时间、地址等基本信息
3. **设置首页内容** - 自定义欢迎语和公告文字
4. **选择测试数据** - 可选择是否填充示例商品和分类（推荐新用户）

完成初始化后，即可使用创建的管理员账户登录系统。

### 测试账户（仅开发环境）

如果您需要测试环境中的默认账户，可以在运行种子数据时设置环境变量：

```bash
CREATE_DEV_ADMIN=true npx tsx prisma/seed.ts
```
...
| 13800138001@example.com | 123456 | 顾客（张小明） |
...
项目提供多种部署方式，根据您的场景选择合适的脚本：

### 部署场景说明

| 场景 | 使用脚本 | 说明 |
|------|---------|------|
| **源码部署** | `install.sh` 或 `deploy.sh` | 有完整源码，需要在本地构建运行 |
| **Docker镜像部署** | `run.sh` | 已构建好镜像，只需拉取运行（推荐生产环境） |
| **Docker Compose** | `deploy.sh` | 本地构建镜像并运行 |

### 方式 1: Docker 镜像部署（推荐生产环境）

使用已构建好的 Docker 镜像，快速部署到任何支持 Docker 的机器。
...
- Git 仓库（脚本会自动从 git remote 获取镜像地址）
...
适合不熟悉命令行的用户，提供交互式配置界面。

```bash
# 源码环境
./install.sh

# Docker 镜像环境（会自动检测并引导使用 run.sh）
./install.sh
```

### 方式 3: Docker Compose 部署（源码环境）

使用源码本地构建 Docker 镜像并运行。
